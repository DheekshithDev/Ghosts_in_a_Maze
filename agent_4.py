...
def agent_4(got_created_numpy_maze):
    unchecked_nodes = []
    checked_nodes = []
    ...
    movable_direc = {'see_left': (0, -1), 'see_right': (0, 1), 'see_up': (-1, 0), 'see_down': (1, 0),
                     'see_diag_top_left': (-1, -1), 'see_diag_bot_left': (1, -1),
                     'see_diag_top_right': (-1, 1), 'see_diag_bot_right': (1, 1)}
    ...
    while len(unchecked_nodes) != 0:
        ...
        if num_of_iterations > max_iterations:
            print("Taking too long to find path. Agent 4 Failed!")
            return None

        cur_node_neighbours = []
        nearby_ghost_in_proximity_pos = []
        ...
        for neigh in cur_node_neighbours:
            ...
            neigh.f = neigh.g + neigh.h - neigh.ghost_value
